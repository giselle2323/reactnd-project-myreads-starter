...
const SearchPage = (props) => {
  ...
  return (
    <div className="search-container">
      <div>
        <input className="search-bar" type="text" placeholder="title, author" value={query} onChange={handleInputChange}/>
      </div>
      <div className="search-results">
        {!matchedBooks.error && matchedBooks.length > 0 &&
          matchedBooks.map(book => <Book key={book.id} book={isBookOnShelf(book) || book} booksShelfChange={handleShelfChange} />)
        }
      </div>
    </div>
  );
